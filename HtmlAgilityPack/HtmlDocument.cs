...
namespace HtmlAgilityPack
{
    ...
    public class HtmlDocument
    {
        ...
        private int _index;
        internal Dictionary<string, HtmlNode> _lastnodes = new Dictionary<string, HtmlNode>();
        private HtmlNode _lastparentnode;
        private int _line;
        private int _lineposition, _maxlineposition;
        internal Dictionary<string, HtmlNode> _nodesid;
        ...
        {
            HtmlNode resetter;
            _lastnodes.TryGetValue(name, out resetter);
            if (resetter == null) 
                return null;
            if (resetter.Closed)
            {
                    ...
                    case ParseState.PcData:
                        ...
                        if ((_currentnode._namelength + 3) <= (_text.Length - (_index - 1)))
                        {
                            if (string.Compare(_text.Substring(_index - 1, _currentnode._namelength + 2),
                                               "</" + _currentnode.Name, StringComparison.CurrentCultureIgnoreCase) == 0)
                            {
        ...
        private bool PushNodeEnd(int index, bool close)
        {
            ...
            if ((close) || (!_currentnode._starttag))
            {
                if ((OptionStopperNodeName != null) && (_remainder == null) &&
                    (string.Compare(_currentnode.Name, OptionStopperNodeName, StringComparison.CurrentCultureIgnoreCase) == 0))
                {
        ...
        private void ReadDocumentEncoding(HtmlNode node)
        {
            ...
            if (att == null)
                return;
            if (string.Compare(att.Value, "content-type", StringComparison.CurrentCultureIgnoreCase) != 0)
                return;
